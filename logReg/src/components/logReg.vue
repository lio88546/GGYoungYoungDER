...
var socket = io.connect("http://192.168.30.117:8542");//與伺服器建立連結
//向伺服器發送的訊息，確認連接狀況良好
socket.emit('condition',{condition:'Client Connection!'});
//接收伺服器發送的訊息，確認連接狀況良好
  ...
  methods:{
    regs(){
      ...
      if(this.check(this.registerData)){
        console.log('access');
        //至server處理
        socket.emit('regs',this.registerData);
      } else {
        alert("ERROR");
      }
    },
